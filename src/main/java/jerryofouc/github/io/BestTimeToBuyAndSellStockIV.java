...
public class BestTimeToBuyAndSellStockIV {
    public static int maxProfit(int k, int[] prices) {
        ...
        int[][] mem = new int[prices.length][k + 1];
        ...
        mem[prices.length - 1][1] = 0;
        for (int i = prices.length - 2; i >= 0; i--) {
            int t = Math.min(k, (prices.length - i)/2);
            for (int j = Math.max(1, t - i - 1); j <= t; j++) {
                int max = getMem(i + 1, j, mem, prices.length);
                for (int q = i + 1; q < prices.length; q++) {
                    if (prices[q] - prices[i] > 0) {
                        int x = prices[q] - prices[i] + getMem(q, j - 1, mem, prices.length);
                        if (x > max) {
                            max = x;
                        }
                    }else {
    ...
    private static int getMem(int i, int j, int[][] mem, int len) {
        if ((mem.length - i) /2 < j) {
            return mem[i][(len - i )/2];
        } else {
            return mem[i][j];
        }
    }
